...
export const fetchAdminItemsAsync = createAsyncThunk(
  'fetchAdminItems/fetchAdminItemsAsync',
  async () => {
    // 通信ごとに配列の中身をか空にする
    ...
    // アイテムidを昇順へと並び替える
    fetchAdminItemsData!.adminItems!.sort(function (
      first: AdminItems,
      second: AdminItems
    ) {
...
export const updateAdminItemsAsync = createAsyncThunk(
  'updateAdminItems/updateAdminItemsAsync',
...
export const adminItemsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      fetchAdminItemsAsync.fulfilled,
      (state, action: PayloadAction<FetchAdminItems>) => {
    ...
    builder.addCase(updateAdminItemsAsync.fulfilled, (state, action: any) => {
      // 削除対象の一意のインデックスを取得
      let deleteIndex: number | undefined = state.adminItems?.findIndex(
        (element) => {
