...
export const fetchUploadItemData = createAsyncThunk(
  'fetchUploadItemData/fetchUploadItemDataAsync',
  async () => {
...
export const uploadItemDataAsync = createAsyncThunk(
  'uploadData/uploadItemDataAsync',
  async (object: any) => {
    ...
    function auth() {
      const uploadTask = strage
        .ref(`images/${object.image.name}`)
        .put(object.image);
...
export const adminSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      uploadItemDataAsync.fulfilled,
      (state, action: PayloadAction<AdminItems>) => {
