  ...
  return (
    <TableRow>
      ...
      <TableCell align='right'>
        {Number(orderList.itemPrice).toLocaleString()}円
      </TableCell>
      <TableCell align='right'>{orderList.itemCount}</TableCell>
      <TableCell align='right'>
        {Number(orderList.itemPrice * orderList.itemCount).toLocaleString()}円
      </TableCell>
      {order.orderItems.length === indexNum + 1 ? (
        <TableCell align='right'>
          {Number(order.totoalPrice).toLocaleString()}円
        </TableCell>
      ) : null}
      ...
      <TableCell align='right'>
        ...
        order.orderItems.length === indexNum + 1 &&
          order.status !== 9 &&
          Number(order.orderDate?.slice(0, 4)) < new Date().getFullYear() ? (
          <>
            <p>配達希望日：{order.orderDate}</p>
            <Button disabled={true} variant='contained'>
              発送済み
        ...
        order.orderItems.length === indexNum + 1 &&
          order.status !== 9 &&
          Number(order.orderDate?.slice(0, 4)) === new Date().getFullYear() &&
          Number(order.orderDate?.slice(5, 7)) < new Date().getMonth() + 1 ? (
          <>
            <p>配達希望日：{order.orderDate}</p>
            <Button disabled={true} variant='contained'>
              発送済み
        ...
        order.orderItems.length === indexNum + 1 &&
          ...
          Number(order.orderDate?.slice(0, 4)) === new Date().getFullYear() &&
          Number(order.orderDate?.slice(5, 7)) === new Date().getMonth() + 1 &&
          Number(order.orderDate?.slice(8, 10)) <= new Date().getDate() ? (
          <>
            <p>配達希望日：{order.orderDate}</p>
            <Button disabled={true} variant='contained'>
              発送済み
          ...
          <>
            ...
            <Button
              onClick={() => {
                cancelOrder(order.orderUniqueId);
              }}
              variant='contained'>
              この注文をキャンセル
            </Button>
