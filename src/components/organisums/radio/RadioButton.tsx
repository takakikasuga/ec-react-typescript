...
export const RadioButton = memo((props: RadioProps) => {
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const selectedItemPrice = (event.target as HTMLInputElement)
  ...
  return (
    <FormControl component='fieldset' style={{ textAlign: 'left' }}>
      ...
      <RadioGroup
        ...
        <FormControlLabel
          value={String(Math.round(detail!.price!.m! * 1.1))}
          control={<Radio data-testid='mPrice' />}
          label={`M-Size：${Number(
            Math.round(detail!.price!.m! * 1.1)
          ).toLocaleString()}円（税込）`}
        />
        <FormControlLabel
          value={String(Math.round(detail!.price!.l! * 1.1))}
          control={<Radio data-testid='lPrice' />}
          label={`L-Size：${Number(
            Math.round(detail!.price!.l! * 1.1)
          ).toLocaleString()}円（税込）`}
        />
      </RadioGroup>
    </FormControl>
